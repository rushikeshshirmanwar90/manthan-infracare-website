...
export function TestimonialsSection() {
  ...
  return (
    <section className="py-20 bg-gradient-to-b from-[#E5E7EB] to-[#2C3E50]/10">
        ...
        <div className="relative">
          <div className="flex flex-wrap justify-center gap-8">
            {visibleTestimonials.map((testimonial) => (
              <div
                key={testimonial.id}
                className="bg-[#E5E7EB] rounded-xl shadow-md p-6 max-w-md flex-1 min-w-[300px] transform transition-all duration-300 hover:shadow-xl"
              >
                <div className="flex items-center mb-4">
                  ...
                  <div>
                    <h3 className="font-bold text-[#2C3E50]">{testimonial.name}</h3>
                    <p className="text-sm text-[#FFB200]">{testimonial.position}</p>
                  </div>
                </div>
                <div className="flex mb-3">
                  {[...Array(5)].map((_, i) => (
                    <Star
                      key={i}
                      size={16}
                      className={i < testimonial.rating ? "text-[#FFB200] fill-[#FFB200]" : "text-[#2C3E50]"}
                    />
                  ))}
                </div>
                <p className="text-[#2C3E50]">{testimonial.text}</p>
              </div>
            ))}
          </div>
          ...
          <button
            ...
            className="absolute left-0 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-[#FFB200] shadow-md text-[#2C3E50] flex items-center justify-center z-10 hover:bg-[#FFB200]/80 transition-colors"
          ...
          <button
            ...
            className="absolute right-0 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-[#FFB200] shadow-md text-[#2C3E50] flex items-center justify-center z-10 hover:bg-[#FFB200]/80 transition-colors"
        ...
        <div className="flex justify-center mt-8 space-x-2">
          {testimonials.map((_, index) => (
            <button
              ...
              className={`w-2.5 h-2.5 rounded-full transition-all ${index === currentIndex ? "bg-[#FFB200] w-6" : "bg-[#2C3E50]"}`}
