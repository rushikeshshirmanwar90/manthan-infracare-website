    ...
    <section className="relative h-screen w-full overflow-hidden pt-16">
        ...
        <div className="max-w-3xl text-white">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <Link
              ...
              className="px-8 py-3 rounded-md bg-[#FFB200] text-[#2C3E50] font-medium hover:shadow-lg transition-all text-center"
            >
              {slides[currentSlide].buttonText}
            </Link>
            <Link
              href="#contact"
              className="px-8 py-3 rounded-md bg-[#2C3E50]/80 backdrop-blur-sm border border-[#E5E7EB]/30 text-[#E5E7EB] font-medium hover:bg-[#2C3E50] transition-all flex items-center justify-center gap-2"
            >
      ...
      <div className="absolute bottom-8 left-0 right-0 flex justify-center gap-2 z-10">
        {slides.map((_, index) => (
          <button
            key={index}
            className={`w-3 h-3 rounded-full transition-all ${index === currentSlide ? "bg-[#FFB200] w-8" : "bg-[#E5E7EB]/50"}`}
